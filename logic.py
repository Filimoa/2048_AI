...
def game_score(mat):
    '''
    function to return score of game
    '''
    score = 0
    for i in range(len(mat)):
        for j in range(len(mat[0])):
...
def up(game):
        ...
        temp,scoreChange=merge(game) # added ScoreChange
        game=temp[0]
        done=done or temp[1]
        game=cover_up(game)[0]
        game=transpose(game)
        # print game
        return (game,done,scoreChange) #now returning ScoreChange

def down(game):
        # print("down ",2)
        game=reverse(transpose(game))
        game,done=cover_up(game)
        temp,scoreChange=merge(game)
        game=temp[0]
        done=done or temp[1]
        game=cover_up(game)[0]
        game=transpose(reverse(game))
        # print game
        return (game,done,scoreChange)
